...
stp_sf <- geojson_sf(here::here("lib", "STPshapefile.json"))
#stp_sf <- geojson_sf("https://openprescribing.net/api/1.0/org_location/?format=json&org_type=stp.json")

stp_sf2 <- stp_sf %>% 
  left_join(stppop, by = c("ons_code" = "stp"))
...
ggplot() +
  ...
  labs(
    ...
    subtitle= "as at 1 February 2020",
    fill = NULL)+
  theme_void()+
  theme(
