...
* `plot_stppop` is the action that runs the R script (defined below). The `run` section tells it which script to run. The `needs` section says that this action cannot be run without having first run the `geenrate_cohort_stppop` action. The `outputs` sections declares that the expected outputs (a log file for debugging, and the two plots) are `moderately_sensitive` and should be considered for release after disclosivity checks.
...
Securiity-wise, this log file is treated as `moderately_sensitive`.
...
Then as before, we define the cohort population and the variables we want to extract within a study definition, define a `cohortextractor` action in the `project.yaml` file, and run using `opensafely run`.
The variables we extract this time are `age`, `sex`, `date_death`, and `death_category`. 
These use some new variable functions and some more expectation definitions, whose details can be found in the documentation [here](https://docs.opensafely.org/en/latest/study_def_intro/).
...
There is also a bump in non-covid deaths, suggesting that identification of covid-related deaths may not be 100\% sensitive, or that health services struggled during this period, or that people we not seeking the care they needed.
