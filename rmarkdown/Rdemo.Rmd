...
It will also create log files in the `metadata/` folder, for debugging. Securiity-wise, this log file is treated as `moderately_sensitive`.
...
Then as before, we define the cohort population and the variables we want to extract within `StudyDefinition()`. 
The variables we extract this time are `age`, `sex`, `date_death`, and `death_category`. 
These use some new extractor functions and some more expectation definitions, whose details can be found in the documentation [here](https://docs.opensafely.org/en/latest/study_def_intro/).
...
This script is run by the Job Runner via the `project.yaml` file, then the outputs are reviewed in the server and released via github. 
